...
APP_TITLE = "👷 EMPLOYEES APP — Cloud (Full, PPE fixed)"
DB = "employees.db"
...
menu = st.sidebar.radio("Menú", OPTIONS_ALL)
    ...
    if st.button("💾 Guardar trabajador"):
        if cid<=0 or crew_id<=0 or not name.strip(): st.warning("Faltan datos.")
            ...
            exec_sql("""INSERT INTO workers(full_name,company_id,crew_id,start_date,gloves_issued_date,sleeves_issued_date,active)
                        VALUES(?,?,?,?,?,?,1)""",                     (name.strip(),cid,crew_id,str(start),str(g_date) if g_chk else None,str(s_date) if s_chk else None))
            st.success("Guardado.")
...
# --- PPE básico en ficha (FIXED) ---
    ...
    if s.strip():
        where += " AND w.full_name LIKE ?"; params.append(f"%{s.strip()}%")

    c, r = q(f"""
        SELECT w.id, w.full_name, c.name, cr.crew_code,
               w.gloves_issued_date, w.gloves_returned_date,
               w.sleeves_issued_date, w.sleeves_returned_date
        FROM workers w
        JOIN companies c ON c.id=w.company_id
        JOIN crews cr     ON cr.id=w.crew_id
        {where}
    ...
    opts = [f"{x[0]} - {x[1]} ({x[2]}/{x[3]})" for x in r]
    sel  = st.selectbox("Trabajador", opts) if opts else None
    wid  = int(sel.split(" - ")[0]) if sel else None
    ...
    if wid:
        ...
        for row in r:
            if row[0] == wid:
                gi, gr, si, sr = row[4], row[5], row[6], row[7]
                break
        ...
        with c1:
            st.markdown("**Gloves**")
            gi_on = st.checkbox("Registrar fecha de entrega (Gloves)", value=(gi is not None), key=f"gi_on_{wid}")
            gi_dt = st.date_input("Fecha entrega Gloves", value=_d(gi), key=f"gi_dt_{wid}", disabled=not gi_on)

            gr_on = st.checkbox("Registrar fecha de devolución (Gloves)", value=(gr is not None), key=f"gr_on_{wid}")
            gr_dt = st.date_input("Fecha devolución Gloves", value=_d(gr), key=f"gr_dt_{wid}", disabled=not gr_on)

        with c2:
            st.markdown("**Sleeves**")
            si_on = st.checkbox("Registrar fecha de entrega (Sleeves)", value=(si is not None), key=f"si_on_{wid}")
            si_dt = st.date_input("Fecha entrega Sleeves", value=_d(si), key=f"si_dt_{wid}", disabled=not si_on)

            sr_on = st.checkbox("Registrar fecha de devolución (Sleeves)", value=(sr is not None), key=f"sr_on_{wid}")
            sr_dt = st.date_input("Fecha devolución Sleeves", value=_d(sr), key=f"sr_dt_{wid}", disabled=not sr_on)

        if st.button("Guardar cambios PPE", key=f"save_ppe_{wid}"):
            exec_sql("""                UPDATE workers SET
                    gloves_issued_date    = ?,
                    gloves_returned_date  = ?,
                    sleeves_issued_date   = ?,
                    sleeves_returned_date = ?
                WHERE id=?
            """, (                str(gi_dt) if gi_on else None,                str(gr_dt) if gr_on else None,                str(si_dt) if si_on else None,                str(sr_dt) if sr_on else None,                wid            ))
            st.success("PPE actualizado.")
            c3, r3 = q("""SELECT gloves_issued_date,gloves_returned_date,
                                 sleeves_issued_date,sleeves_returned_date
                          FROM workers WHERE id=?""", (wid,))
            st.caption("Valores guardados:")
            st.write(to_dicts(c3, r3)[0])
